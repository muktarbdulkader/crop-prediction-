...
import type { PredictionParams, PredictionResult, Language, LeafAnalysisResult, User } from '../types';
...
const getLanguageName = (lang: Language): string => {
    switch (lang) {
        ...
        case 'om': return 'Oromo (Afaan Oromoo)';
        default: return 'English';
    }
}

export const predictCrop = async (ai: GoogleGenAI, params: PredictionParams, language: Language): Promise<PredictionResult> => {
  ...
  const prompt = `
    ...
    CRITICAL INSTRUCTION: Your entire response, including all text fields, MUST be ONLY in ${getLanguageName(language)}. Do not use any other language.
  `;
  ...
  try {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            reason: {
              type: Type.STRING,
              description: "A brief justification for why this crop is suitable based on the provided data."
            },
            confidence: {
              type: Type.NUMBER,
            ...
            expectedYield: {
              type: Type.STRING,
              description: "An estimated range of the expected yield in kg/ha (e.g., '2500-3500 kg/ha')."
            },
            marketDemand: {
              type: Type.STRING,
              description: "A brief analysis of the market demand for the crop in the region (e.g., 'High local demand')."
            }
          },
    ...
    if (typeof result.crop !== 'string' || typeof result.reason !== 'string' || typeof result.confidence !== 'number') {
...
export const askAgriBotStream = async (ai: GoogleGenAI, message: string, language: Language, onChunk: (chunk: string) => void) => {
  try {
    ...
    if (!chat) {
        chat = ai.chats.create({
            ...
            config: {
                systemInstruction: `You are AgriBot, a helpful AI assistant for farmers and agricultural experts. Answer questions concisely and accurately about farming, crops, soil, plant diseases, and sustainable agriculture practices. Use clear and simple language. Use Markdown for formatting lists, bolding, etc. when it improves readability. CRITICAL INSTRUCTION: Your entire response must be ONLY in ${getLanguageName(language)}. Do not use any other language.`,
            },
        });
    }
...
export const analyzeLeaf = async (ai: GoogleGenAI, imageBase64: string, mimeType: string, prompt: string, language: Language): Promise<LeafAnalysisResult> => {
    const imagePart = {
        inlineData: {
            data: imageBase64,
            mimeType,
    ...
    const textPart = {
        ...
        Your response must be a JSON object adhering to the provided schema.
        CRITICAL INSTRUCTION: Your entire response, especially the 'analysis' field, MUST be ONLY in ${getLanguageName(language)}. Do not use any other language.
        `,
    };
    ...
    try {
            ...
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        analysis: {
                            type: Type.STRING,
                            description: "The detailed analysis of the leaf condition in Markdown format."
                        },
                        confidence: {
                            type: Type.NUMBER,
                            description: "A confidence score for the diagnosis, from 0 to 100."
                        }
                    },
        ...
        if (typeof result.analysis !== 'string' || typeof result.confidence !== 'number') {
            throw new Error("Invalid response format from AI model for leaf analysis.");
        }
...
export const analyzeSoil = async (ai: GoogleGenAI, imageBase64: string, mimeType: string, prompt: string, language: Language, plan: User['plan']): Promise<string> => {
    const imagePart = {
        inlineData: {
            data: imageBase64,
            mimeType,
    ...
    const amendmentsSection = plan === 'pro'
      ...
      : `- **Cost-Effective Amendments:** (⭐ Upgrade to Pro to unlock detailed, practical advice on improving your soil with low-cost materials.)`;
    ...
    const textPart = {
        ...
        CRITICAL INSTRUCTION: Your entire response, including all headings and content, MUST be ONLY in ${getLanguageName(language)}. Do not use any other language.`,
    };
...
export const analyzeProduct = async (ai: GoogleGenAI, imageBase64: string, mimeType: string, prompt: string, language: Language, plan: User['plan']): Promise<string> => {
    const imagePart = {
        inlineData: {
            data: imageBase64,
            mimeType,
    ...
    const safetySection = plan === 'pro'
        ? `- **Safety Precautions:** (List key safety warnings, required protective equipment, and first aid info.)`
        : `- **Safety Precautions:** (⭐ Upgrade to Pro to unlock critical safety warnings, protective equipment requirements, and first-aid information for this product.)`;

    const textPart = {
        text: `Act as an expert on agricultural products (like pesticides, fertilizers, etc.). Analyze the provided image of a product label or packaging.
        ${prompt}.
        Your response MUST be a well-structured Markdown format with these exact headings if applicable:
        - **Product Identification:** (e.g., Name, Type like Fungicide, Herbicide, Fertilizer)
        - **Active Ingredients:** (List the primary active chemicals and their concentrations if visible)
        - **Primary Use:** (What is this product for? Which crops/pests?)
        - **Application Instructions:** (Summarize dosage, mixing, and application method)
        - **Manufacturing Date:** (If visible on the label)
        - **Expiry Date:** (If visible on the label)
        ${safetySection}
    ...
    try {
        ...
        throw new Error("PRODUCT_ANALYSIS_FAILED");
    }
};

export const getTreatmentPlan = async (ai: GoogleGenAI, analysis: string, language: Language): Promise<string> => {
    const prompt = `
...
export const generateSpeech = async (ai: GoogleGenAI, text: string): Promise<AudioBuffer> => {
    if (!text || !text.trim()) {
    ...
    try {
        ...
        if (text.length > MAX_TTS_LENGTH) {
            console.warn(`TTS input text truncated from ${text.length} to ${MAX_TTS_LENGTH} characters.`);
        ...
        if (!base64Audio) {
            console.error("Speech generation failed. API Response:", JSON.stringify(response, null, 2));
...
export const generateFarmingGuide = async (ai: GoogleGenAI, crop: string, region: string, language: Language): Promise<string> => {
    const prompt = `
        ...
        CRITICAL INSTRUCTION: Your entire response, including all Markdown headings and content, MUST be ONLY in ${getLanguageName(language)}. Do not use any other language.
    `;
...
export const getLocationData = async (ai: GoogleGenAI, lat: number, lon: number): Promise<{ region: string; temperature: number }> => {
  const prompt = `
  ...
  try {
    ...
    const validRegions = ['Oromia', 'Amhara', 'SNNPR', 'Tigray', 'Somali', 'Afar'];
    if (typeof result.region !== 'string' || !validRegions.includes(result.region) || typeof result.temperature !== 'number') {
...
export const extractDiseaseName = async (ai: GoogleGenAI, analysis: string, language: Language): Promise<string> => {
    const prompt = `
        ...
        **Analysis Text:**
        ...
        CRITICAL INSTRUCTION: Your entire response must be ONLY in ${getLanguageName(language)}.
    `;
    try {
        const response = await ai.models.generateContent({
            model: "gemini-2.5-flash",
        ...
        return response.text.trim().replace(/\*/g, ''); // Remove any markdown bolding
    } catch (error) {
...
export const generateDiseaseImage = async (ai: GoogleGenAI, diseaseName: string, plantName: string): Promise<string> => {
...
export const getPlantGrowthStages = async (ai: GoogleGenAI, plantName: string, language: Language): Promise<string> => {
    const prompt = `
