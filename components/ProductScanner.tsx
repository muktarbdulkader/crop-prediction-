...
const ProductScanner: React.FC<ProductScannerProps> = ({ language, t, user, onUpgrade, onAskAgriBot }) => {
    ...
    const [isProFeatureLocked, setIsProFeatureLocked] = useState(false);
    const justUpgradedRef = useRef(false);

    useEffect(() => {
        setPrompt(t.productScannerDefaultPrompt || 'Analyze this product.');
    }, [t.productScannerDefaultPrompt, language]);

    useEffect(() => {
        if (user.plan === 'pro' && justUpgradedRef.current) {
            justUpgradedRef.current = false;
            handleSubmit(); // Re-run analysis with pro plan
        }
    }, [user.plan]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const userPrompt = prompt || t.productScannerDefaultPrompt;
            const result = await analyzeProduct(ai, imageBase64, image.file.type, userPrompt, language, user.plan);
            
            if (user.plan === 'free' && result.includes('Upgrade to Pro')) {
                setIsProFeatureLocked(true);
            }
    ...
    const handlePaymentSuccess = () => {
        ...
        // The useEffect will now trigger a re-analysis
    };
    ...
    return (
        <div>
            <CameraCapture
                ...
                t={t}
                helperText={t.cameraHelperTextProduct}
            />
             {isPaymentModalOpen && (
                 <PaymentModal 
            ...
            <h2 className="text-2xl font-bold mb-4 text-brand-green-dark border-b-2 border-brand-green/50 pb-3">{t.productScannerTitle}</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <div className="p-4 border-2 border-dashed rounded-lg border-base-300 bg-base-100">
                        <label 
                            htmlFor="product-dropzone-file"
                        ...
                        >
                            {image ? (
                                <img src={image.previewUrl} alt="Product preview" className="h-full w-full max-h-64 object-contain rounded-lg p-2" />
                            ) : (
                            ...
                            <input id="product-dropzone-file" type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e.target.files)} />
                        </label>
                    </div>
                    ...
                    {image && (
                            ...
                            <div>
                                ...
                                {showPromptInput && (
                                     <textarea
                                        ...
                                        placeholder={t.productAnalysisPrompt}
                     ...
                     {analysis && !isLoading ? (
                        ...
                        >
                            {isLoading ? t.aiAnalyzingProduct : t.soilAnalysisButton}
                        </button>
                    )}
                </div>
                <div className="space-y-4">
                    <div className="bg-card border border-base-200 p-4 rounded-lg min-h-[200px] flex flex-col shadow-lg">
                        <div className="flex justify-between items-center mb-2">
                            <h3 className="text-xl font-bold text-brand-green-dark">{t.productAnalysisResult}</h3>
                            {analysis && rawAnalysis && <PlayAudioButton textToRead={rawAnalysis} language={language} t={t} />}
                        </div>
                        <div className="flex-grow overflow-y-auto pr-2">
                            {isLoading && <LoadingIndicator text={t.aiAnalyzingProduct} />}
                            ...
                            {!analysis && !isLoading && !error && (
                                <div className="text-center text-text-muted pt-10">
                                    <p>{t.uploadPrompt} an image of a product to begin.</p>
                                </div>
                            )}
                        </div>
                         {isProFeatureLocked && !isLoading && (
                            <div className="mt-4 p-3 bg-yellow-100 border border-yellow-300 rounded-lg text-center animate-fade-in">
                                <p className="text-sm text-yellow-800 font-semibold">{t.proFeatures.productSafety}</p>
                                <button onClick={handleUpgradeClick} className="mt-2 bg-brand-green hover:bg-brand-green-dark text-white text-sm font-bold py-1.5 px-4 rounded-lg transition-colors">
                                    {t.proFeatures.upgradeButton}
                                </button>
                            </div>
                        )}
                    </div>
