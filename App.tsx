...
const NavButton: React.FC<{
  ...
  >
    ...
    <div
      className={`absolute bottom-0 left-0 right-0 h-1 bg-brand-green rounded-full transition-transform duration-300 ease-in-out ${
...
const App: React.FC = () => {
  ...
  const handleLogin = (loggedInUser: Omit<User, "plan">) => {
    setUser({ ...loggedInUser, plan: "free" }); // All users start on free plan
    setView("app");
  };
  ...
  if (view !== "app" || !user) {
    return (
      <div className="min-h-screen bg-brand-bg text-text-main p-4 sm:p-6 lg:p-8 flex flex-col justify-center items-center">
        ...
        {view === "landing" && (
          <LandingPage onNavigateToAuth={() => setView("auth")} t={t} />
        )}
        {view === "auth" && (
          <AuthPage
            onLogin={handleLogin}
          ...
          <div className="absolute top-0 right-0 sm:static flex items-center gap-4 mt-2 sm:mt-0">
            ...
            <div className="hidden sm:flex items-center gap-2 text-sm">
              <span className="font-semibold text-text-muted">
                {t.welcomeMessage}, {user.name.split(" ")[0]}!
              </span>
              <button
                onClick={handleLogout}
                className="bg-brand-brown/80 text-white px-3 py-1 rounded-full hover:bg-brand-brown transition-colors"
              >
                {t.logout}
              </button>
            </div>
            ...
            icon={
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3 8c2-3 6-3 8 0s6 3 8 0"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3 12c2-3 6-3 8 0s6 3 8 0"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M3 16c2-3 6-3 8 0s6 3 8 0"
                />
              </svg>
            }
