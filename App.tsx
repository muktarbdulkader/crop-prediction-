...
const App: React.FC = () => {
  ...
  const [activeTool, setActiveTool] = useState<Tool>(() => {
    ...
    return (savedTool === 'predictor' || savedTool === 'chatbot' || savedTool === 'scanner' || savedTool === 'soil_scanner' || savedTool === 'profile') ? savedTool as Tool : 'predictor';
  });
  ...
  const renderTool = () => {
    ...
    switch (activeTool) {
      ...
      case 'scanner':
        return <LeafScanner user={user} onNavigateToProfile={() => setActiveTool('profile')} {...props} />;
      case 'soil_scanner':
        ...
        <header className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          ...
          <div className="absolute top-0 right-0 sm:static flex items-center gap-4 mt-2 sm:mt-0">
              ...
              <div className="hidden sm:flex items-center gap-2 text-sm">
                  <span className="font-semibold text-text-muted">{t.welcomeMessage}, {user.name.split(' ')[0]}!</span>
